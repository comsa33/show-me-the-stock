...
class StockDataFetcher:
    ...
    def get_stock_data(_self, symbol: str, period: str = '1y', market: str = 'US') -> Optional[pd.DataFrame]:
        try:
            if market == 'KR':
                if symbol in _self.kospi_symbols.values():
                    data = fdr.DataReader(symbol, start=datetime.now() - timedelta(days=365))
                else:
                    return None
            else:
    ...
    @st.cache_data(ttl=300)
    def get_multiple_stocks(_self, symbols: List[str], period: str = '1y', market: str = 'US') -> Dict[str, pd.DataFrame]:
        results = {}
